...
.setDescription(`<a:cryptored:770187639801774080> **HATA** <a:cryptored:770187639801774080> - Bu sunucuda yetkili değilsin.`)
      ...
      message.channel.send(`<a:cryptored:770187639801774080> **Başarılı ile reklam engel kapandı.**`).then(l => {
      l.delete({timeout: 5000})
    })
    }else{
      db.set(`reklam.${message.guild.id}.durum`,true)
      message.channel.send(`<a:cryptoonay:770187690402250772> **Başarılı ile reklam engel açıldı.**`).then(l => {
      l.delete({timeout: 5000})
    })
    }
    }
